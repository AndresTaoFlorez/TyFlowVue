...
async function run() {
  ...
  // Debug: inspect bar elements
  const debug = await page.evaluate(() => {
    const bars = document.querySelectorAll('.wgb__bar')
    const results = []
    bars.forEach(bar => {
      const rect = bar.getBoundingClientRect()
      const nameEl = bar.querySelector('.wgb__bar-name')
      const initialsEl = bar.querySelector('.wgb__bar-initials')
      const styles = window.getComputedStyle(bar)

      const info = {
        barWidth: rect.width,
        barHeight: rect.height,
        barColor: styles.getPropertyValue('--bar-color'),
        barBg: styles.backgroundColor,
      }

      if (nameEl) {
        const ns = window.getComputedStyle(nameEl)
        info.nameDisplay = ns.display
        info.nameText = nameEl.textContent
        info.nameFontSize = ns.fontSize
        info.nameColor = ns.color
      }
      if (initialsEl) {
        const is = window.getComputedStyle(initialsEl)
        info.initialsDisplay = is.display
        info.initialsText = initialsEl.textContent
        info.initialsFontSize = is.fontSize
        info.initialsColor = is.color
      }
  ...
  console.log('=== BAR DEBUG (Week View) ===')
  debug.forEach((d, i) => console.log(`Bar ${i}:`, JSON.stringify(d, null, 2)))
