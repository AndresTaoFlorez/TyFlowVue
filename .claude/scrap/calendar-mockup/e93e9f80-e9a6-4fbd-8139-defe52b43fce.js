...
// ---- Especialistas (color propio + capacidad semanal objetivo en horas) ----
const SPECIALISTS = [
  { id: 'sp-cc', fullName: 'Carla Castro',     color: '#E0675F', cap: 40 },
  { id: 'sp-pe', fullName: 'Pablo Emiliano',   color: '#4F8DF7', cap: 40 },
  { id: 'sp-sr', fullName: 'Santiago Rendón',  color: '#9B6DF3', cap: 40 },
  { id: 'sp-dm', fullName: 'Diana Marín',      color: '#1FA888', cap: 40 },
  { id: 'sp-mv', fullName: 'Mateo Vargas',     color: '#E0A23B', cap: 40 },
  { id: 'sp-lo', fullName: 'Lucía Ortega',     color: '#21B3C6', cap: 40 },
];

// ---- Festivos (ISO -> etiqueta). Bloquean / avisan al agendar. ----
const HOLIDAYS = {
  '2026-06-12': 'Corpus Christi',
};
function holidayFor(iso) { return HOLIDAYS[iso] || null; }
function isWeekendDow(dow) { return dow >= 5; }
...
// Carga por especialista sobre un conjunto de ventanas (solo activas cuentan).
// Devuelve [{ spec, hours, cap, ratio, level }] ordenado de mayor a menor carga.
function specialistLoad(windows) {
  return SPECIALISTS.map(spec => {
    const hours = windows
      .filter(w => w.specialistId === spec.id && w.active)
      .reduce((s, w) => s + durationHrs(w), 0);
    const cap = spec.cap || 40;
    const ratio = cap ? hours / cap : 0;
    const level = ratio > 1 ? 'high' : ratio >= 0.6 ? 'mid' : 'low';
    return { spec, hours, cap, ratio, level };
...
// Agrupa ventanas idénticas (mismo día + inicio + fin + app + estado) en un
// solo bloque. Escala a 100+ especialistas con el mismo turno → una tarjeta.
function groupWindows(windows) {
  const map = new Map();
  for (const w of windows) {
    const key = `${w.day}|${w.start}|${w.end}|${w.appId}|${w.active ? 1 : 0}`;
  ...
  for (const [key, members] of map) {
    const f = members[0];
    groups.push({
      id: 'g_' + key, day: f.day, start: f.start, end: f.end,
      appId: f.appId, active: f.active, specialistId: f.specialistId,
      members, count: members.length, grouped: members.length > 1,
    });
  }
  return groups;
}
...
window.TY = {
  APPS, SPECIALISTS, WINDOWS, WEEK_DATES, HOURS, HOLIDAYS,
  appById, specById, initialsOf, fmtHour, fmtRange, durationHrs,
  holidayFor, isWeekendDow, specialistLoad,
  DAY_LABELS, DAY_FULL, MONTHS, ALLDAY_THRESHOLD,
  layoutColumns, splitDay, groupWindows, buildMonthGrid,
};
})();
